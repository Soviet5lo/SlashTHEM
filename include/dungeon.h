...
#define Is_mtemple_level(x)	(on_level(x, &mtemple_level))
#define Is_forge_level(x)	(on_level(x, &forge_level))
#define Is_hitch_level(x)	(on_level(x, &hitch_level))
#define Is_compu_level(x)	(on_level(x, &compu_level))
#define Is_key_level(x)	(on_level(x, &key_level))
...
typedef struct mapseen_feat {
	...
	*/
	...
	Bitfield(nshop, 2);
	Bitfield(ntemple, 2);
	/* [BarclayII] SLASH'EM added some shop types */
	Bitfield(shoptype, 6);
