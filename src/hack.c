...
boolean guess;
{
	...
	while (n != 0) {
		...
		int dirmax = (u.umonnum == PM_GRID_BUG || u.umonnum == PM_GRID_XORN )? 4 : 8;
...
lookaround()
{
    ...
    if ((u.umonnum == PM_GRID_BUG || u.umonnum == PM_GRID_XORN)&& u.dx && u.dy) {
	nomul(0);
	return;
    }
	...
	if((u.umonnum == PM_GRID_BUG || u.umonnum == PM_GRID_XORN)&& x != u.ux && y != u.uy) continue;
...
maybe_wail()
{
    ...
    wailmsg = moves;
    if (Role_if(PM_WIZARD) || Role_if(PM_CONVICT) || Role_if(PM_COURIER) || Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_WARRIOR) || Role_if(PM_ELPH) || Race_if(PM_ELF) || Race_if(PM_ASGARDIAN) || Race_if(PM_HERETIC) || Race_if(PM_ALBAE) || Role_if(PM_VALKYRIE)) {
	const char *who;
	int i, powercnt;

	who = (!rn2(10)) ? urace.noun :(Role_if(PM_WIZARD) || Role_if(PM_CONVICT) || Role_if(PM_COURIER) || Role_if(PM_SPACEWARS_FIGHTER) || Role_if(PM_WARRIOR) || Role_if(PM_ELPH) || Race_if(PM_ASGARDIAN) || Race_if(PM_HERETIC) || Race_if(PM_ALBAE) || Role_if(PM_VALKYRIE)) ?
		urole.name.m : "Elf";
	if (u.uhp == 1) {
