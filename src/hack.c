...
{
	if (is_pool(u.ux, u.uy, FALSE)) return FALSE;

	if (is_pool(x, y, FALSE) && !Levitation && !Flying && !Confusion && !Stunned && levl[x][y].seenv) return TRUE; 
...
maybe_wail()
{
    ...
    wailmsg = moves;
    if (Role_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE) || Role_if(PM_WARRIOR)) {
	const char *who;
	int i, powercnt;

	who = (!rn2(10)) ? urace.noun :(Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE) || Role_if(PM_WARRIOR)) ?
		urole.name.m : "Elf";
	if (u.uhp == 1) {
