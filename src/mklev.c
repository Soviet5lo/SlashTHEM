...
mineralize()
{
	...
	for (x = 1; x < (COLNO); x++)
		...
		if ((levl[x][y].typ == CORR && !rn2(2000 / level_difficulty() )) )
			makemon((struct permonst *)0, x, y, NO_MM_FLAGS);

		if ((levl[x][y].typ == ICE && !rn2(500 / level_difficulty() )) )
			makemon((struct permonst *)0, x, y, NO_MM_FLAGS);

		if ((levl[x][y].typ == AIR && !rn2(1000 / level_difficulty() )) )
			makemon((struct permonst *)0, x, y, NO_MM_FLAGS);
