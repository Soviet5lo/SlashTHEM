...
struct monst *mtmp;
{
	...
	/* Note: trap doors take precedence over teleport traps. */
		...
		if (isok(xx,yy))
		if (xx != u.ux && yy != u.uy)
		if ((mtmp->data != &mons[PM_GRID_BUG] && mtmp->data != &mons[PM_GRID_XORN]) || xx == x || yy == y)
...
/*WAC Handled later
		if (otmp->otyp == WAN_FIREBALL) {
		    buzz((int) -10 - SPE_FIREBALL - SPE_MAGIC_MISSILE, rn2(2)+3 + (rnd(level_difficulty()) / 5) ,
		    mtmp->mx, mtmp->my,
...
*/
		...
		buzz((int)(-30 - (otmp->otyp - WAN_MAGIC_MISSILE)),
			(otmp->otyp == WAN_MAGIC_MISSILE) ? 2 + (rnd(level_difficulty()) / 10) : (otmp->otyp == WAN_SOLAR_BEAM) ? 8 + (rnd(level_difficulty()) / 4) : 6 + (rnd(level_difficulty()) / 8),
			mtmp->mx, mtmp->my,
			sgn(mtmp->mux-mtmp->mx), sgn(mtmp->muy-mtmp->my));
		m_using = FALSE;
/*                }*/
	...
	if (!stuck && !immobile && !mtmp->cham && monstr[monsndx(mdat)] < 6) {
	  ...
	  for(xx = x-1; xx <= x+1; xx++)
		...
		if (isok(xx,yy) && (xx != u.ux || yy != u.uy))
		    if ((mdat != &mons[PM_GRID_BUG] && mdat != &mons[PM_GRID_XORN] ) || xx == x || yy == y)
			if (/* (xx==x && yy==y) || */ !level.monsters[xx][yy])
			    if ((t = t_at(xx, yy)) != 0 &&
