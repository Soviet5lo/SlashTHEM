	...
	struct obj *obj;
{
	...
	switch (rnd(6))
	{
		...
		case 5:
			/* Catch your dinner */
			if (fishing && flags.cram_count < 50 && !rn2(10) && /*much lower chance to slow down farming --Amy*/
	...
	if (mtmp) {
		...
		pline("Your weapon shatters into pieces!");
		useup(obj);
		return (1);
		}
		else {(void) thitmonst(mtmp, uwep, 1);}
		return (1);
	}
...
potion_charge_cost(struct obj *pot)
{
	...
	cost = objects[pot->otyp].oc_cost / 150;
	if (cost < 1) cost = 1;
	return(cost);
}
