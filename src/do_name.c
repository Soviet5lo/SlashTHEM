 ...
 * This is only used for mon_wounds, this may change in the future
 */
STATIC_OVL char *
nextmbuf()
{
...
static const char * const bogusmons[] = {
	...
	/*"microscopic space fleet", "Ravenous Bugblatter Beast of Traal",*/ /* Real monsters now! */
						/* HGttG */
...
struct monst *mon;
{
     ...
     * Healers are skilled in medical knowledge
     * Necromancers and Undertakers understand corpses very well
     */
 ...
 * It will only print something if the monster's wound adjective hanges (e.g.
...
int pre_wound_hp;
{
    ...
    if (!Role_if(PM_HEALER) && !Role_if(PM_NECROMANCER) && !Role_if(PM_UNDERTAKER)) {
        return; /* Optimization; redundant with check in mon_wounds */
    }
