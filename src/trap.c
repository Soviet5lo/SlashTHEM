...
unsigned trflags;
{
	...
	switch(ttype) {
...
#endif
		if (thitu(8, dmgval(otmp, &youmonst) + rnd((level_difficulty() / 2) + 1) , otmp, "arrow")) {
		...
		if (isok(trap->launch.x,trap->launch.y) && IS_STWALL(levl[trap->launch.x][trap->launch.y].typ)) {
		    buzz(trap->launch_otyp, 7 + rnd((level_difficulty() / 10) + 1) ,
...
#endif
		if (thitu(7, dmgval(otmp, &youmonst) + rnd((level_difficulty() / 3) + 1), otmp, "little dart")) {
		    if (otmp->opoisoned)
		...
		if (trap->once && trap->tseen && !rn2(15)) {
		    ...
		    int dmg = d(2,6) + rnd((level_difficulty() * 2 / 3) + 1); /* should be std ROCK dmg? */
		...
		fall_asleep(-rnd(25 + rnd((level_difficulty() / 4) + 1) ), TRUE);
#ifdef STEED
...
glovecheck:		(void) rust_dmg(uarmg, "gauntlets", 1, TRUE, &youmonst);
...
#endif
		if (ttype == SPIKED_PIT) {
		    losehp(rnd(10) + rnd((level_difficulty() / 2) + 1) ,"fell into a pit of iron spikes",
			NO_KILLER_PREFIX);
		    if (!rn2(6))
			poisoned("spikes", A_STR, "fall onto poison spikes", 8);
		} else
		    losehp(rnd(6) + rnd((level_difficulty() / 3) + 1) ,"fell into a pit", NO_KILLER_PREFIX);
		...
		if (thick_skinned(youmonst.data)) {
			...
			losehp(rnd(10)+ rnd(level_difficulty() + 1),"sharpened bamboo stick",KILLED_BY_AN);
		}
		break;
...
int zx,zy;
{
	...
	losehp(dmg + rnd((level_difficulty()*2) + 1 ),"shower of rocks",KILLED_BY_AN);
...
{
        int num = 0;
        num = d(4, 4) + rnd((level_difficulty() / 3) + 1);
        if (box) {
        ...
        if (Cold_resistance) {
                shieldeff(u.ux, u.uy);
                num = d(2, 2) + rnd((level_difficulty() / 5) + 1);
        }
 ...
 */
	...
	if (!num)
	    ...
	    losehp(num + rnd((level_difficulty() / 3) + 1), tower_of_flame, KILLED_BY_AN);
	burn_away_slime();
...
register boolean force, here;
{
	 ...
	 * Alignment keys, too, even though the game is still winnable without them. */
			...
			if ( (!rn2(50) || force ) && obj->otyp != SPE_BOOK_OF_THE_DEAD && obj->otyp != AMULET_OF_YENDOR && obj->otyp != CANDELABRUM_OF_INVOCATION && obj->otyp != BELL_OF_OPENING && obj->oartifact != ART_KEY_OF_LAW && obj->oartifact != ART_KEY_OF_CHAOS && obj->oartifact != ART_KEY_OF_NEUTRALITY   ) { /* 2% chance for each item to be affected */
