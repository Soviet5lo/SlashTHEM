...
register struct obj *food;
{
	...
	if (u.uhs != SATIATED) {
		if (!food || food->otyp != AMULET_OF_STRANGULATION)
			return;
	} else if ((Role_if(PM_KNIGHT) && u.ualign.type == A_LAWFUL) || Role_if(PM_PALADIN)) {
...
register int pm;
{
	...
	switch(pm) {
		...
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WERERAT;
		break;
	    case PM_HUMAN_WEREJACKAL:
		catch_lycanthropy = TRUE;
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WEREJACKAL;
		break;
	    case PM_HUMAN_WEREWOLF:
		catch_lycanthropy = TRUE;
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WEREWOLF;
		break;
	    case PM_HUMAN_WEREPANTHER:            
		catch_lycanthropy = TRUE;
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WEREPANTHER;
		break;
	    case PM_HUMAN_WERETIGER:
		catch_lycanthropy = TRUE;
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WERETIGER;
		break;
	    case PM_HUMAN_WERESNAKE:
		catch_lycanthropy = TRUE;
		if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC)) u.ulycn = PM_WERESNAKE;
		break;
	...
	if (!Race_if(PM_HUMAN_WEREWOLF) && !Role_if(PM_LUNATIC) &&
...
gluttonous()
{
	...
	if ((u.uhs == SATIATED) && ((Role_if(PM_KNIGHT) && u.ualign.type == A_LAWFUL) || Role_if(PM_PALADIN))) {
			adjalign(-3);		/* gluttony is unchivalrous */
		You("feel like a glutton!");        
	}
}
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	if(tin.tin->spe != 1) {
	    ...
	    You("consume %s %s.", tintxts[r].txt,
			...
			if((r==SZECHUAN_TIN && tin.tin->selfmade == FALSE) && rn2(2)) {
...
/* This is based on an old Usenet joke, a fake a.out manual page */
 ...
 give_feedback:
		    bad_for_you = otmp->cursed ||
		      ((Race_if(PM_HUMAN_WEREWOLF) || Role_if(PM_LUNATIC)) &&
...
register struct obj *otmp;
{
	switch(otmp->otyp) {
		...
		if (u.ulycn >= LOW_PM || is_were(youmonst.data) || Race_if(PM_HUMAN_WEREWOLF) || Role_if(PM_LUNATIC) )
		    you_unwere(TRUE);
		break;
	    case HOLY_WAFER:            
...
#endif /* CONVICT */
		/* 5lo: So can vampires and ghouls, since they're undead and can sometimes be
		 * forced to go long periods of time without feeding */
		&& (!(Race_if(PM_VAMPIRE) || Race_if(PM_GHOUL)) || (moves % 2) || (u.uhs < HUNGRY))
